...
{
    public class OpusHandler : IMediaHandler // should be Opus in Ogg (see https://wiki.xiph.org/OggOpus)
    {
        ...
        public const string MEDIA_TYPE = "opus";
        ...
        public void HandleInput(IMediaServer mediaServer, List<MediaChannel> channels, Stream inputStream, string mediaInfo)
        {
            try
            {
                ...
                try
                {
                    while (mediaServer.IsWorking())
                    {
                        ...
                        for (var currentIdx = 0; currentIdx < currentSize; currentIdx++)
                        {
                            ...
                            {
                                mediaServer.Logger.Error("Exceeded max cache size when read OPUS(OGG) input.");
                                foundError = true;
                                break;
                            }
                        }
                ...
                {
                    mediaServer.Logger.Error("OPUS(OGG) input error: " + ex.Message);
                }
            }
            catch (Exception ex)
            {
                mediaServer.Logger.Error("OPUS(OGG) input process error in thread: " + ex.Message);
            }
        }
    }
}
